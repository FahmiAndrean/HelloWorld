...
class Talk(object):
    ...
    @loggedIn
    def sendMention(self, to, name, text="", mids=[]):
        arrData = ""
        arr = []
        mention = "@fahmiadrn. "
        if mids == []:
        ...
        return self.sendMessage(to, textx, {'AGENT_NAME': name, 'AGENT_LINK': 'line://ti/p/{}'.format(self.getUserTicket().id), 'AGENT_ICON': "http://dl.profile.line-cdn.net/" + self.getProfile().picturePath, 'MENTION': str('{"MENTIONEES":' + json.dumps(arr) + '}')}, 0)
    ...
    @loggedIn
    def sendMessageFooter(self, to, name, text):
        self.sendMessage(to, text, {'AGENT_NAME': name, 'AGENT_LINK': 'line://ti/p/{}'.format(self.getUserTicket().id), 'AGENT_ICON': "http://dl.profile.line-cdn.net/" + self.getProfile().picturePath})
    ...
    def sendImageWithFooter(self, to, url, link, icon, footer):
        path = self.downloadFileURL(url, 'path')
        self.sendImageFooter(to, path, link, icon, footer)
        return self.deleteFile(path)
